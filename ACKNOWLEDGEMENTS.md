...
| --- | --- | --- |
| [DejaVu Sans Mono](https://dejavu-fonts.github.io/) | The 8x16 bitmap glyphs the PNG screenshot writer draws text with (`src/render/screenshot_font_data.rs`, generated by `tools/make_screenshot_font.py`) | Bitstream Vera Fonts Copyright + DejaVu changes — permissive, attribution only |

Only rasterized glyph bitmaps are embedded, not the font files. Box
drawing, block elements, braille, and sextants are NOT taken from the
font: the engine draws those geometrically so their strokes meet
exactly at every cell boundary.
