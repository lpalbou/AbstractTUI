...
| [api.md](api.md) | The public API surface, module by module — including the focus rule (who receives keys, and how a field asks for the caret at startup), Codex-compatible text editing chords, selection-safe follow-tail scrolling, List row-context requests with the reusable `ContextMenu`, and the `DrawerDock` drawer rail with its portable stacked-label boundary. |
...
- [`../examples/README.md`](../examples/README.md) — the examples
  catalog, ordered as a learning path: 32 runnable programs from
